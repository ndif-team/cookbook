...
def test_calculate_neuron_input_weights(
    ...
    probe = t.randn(model.d_model, 8, 8, device=device)

    # W_in has shape [d_model, d_mlp], so column neuron gives [d_model]
...
def test_calculate_neuron_output_weights(
    ...
    # W_out has shape [d_mlp, d_model], so row neuron gives [d_model]
