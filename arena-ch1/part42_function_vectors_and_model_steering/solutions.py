...
arena_ch1_dir = section_dir.parent
if str(arena_ch1_dir) not in sys.path:
    sys.path.append(str(arena_ch1_dir))
...
if MAIN:
    model = StandardizedTransformer(
        "openai-community/gpt2-xl", dtype=t.float16, device_map="auto", check_renaming=False
    )
...
if MAIN:
    ...
    # Split sentence into tokens (note we see the special G character in place of prepended spaces).
...
if MAIN:
    ...
    with model.trace(prompt):
        # Save the model's hidden states using nnterp's layers_output accessor
        hidden_states = model.layers_output[-1].save()

        # Save the model's logit output using nnterp's logits accessor
...
if MAIN:
    # For GPT-2-XL, we compute attention patterns manually since the dropout-based
    # extraction from GPT-J doesn't work the same way. We can use the model's
    # attention mechanism directly.
    with model.trace(prompt):
        # Access the attention output with attention weights
        # GPT-2 stores q, k, v concatenated in c_attn
        q_proj = model.layers[0].self_attn.c_attn.output.save()

    # For GPT-2, c_attn output has shape (batch, seq, 3 * d_model) containing Q, K, V concatenated
    q, k, v = q_proj[0].split(D_MODEL, dim=-1)  # each is (seq, d_model)

    # Reshape to heads
    q = q.view(-1, N_HEADS, D_HEAD).transpose(0, 1)  # (n_heads, seq, d_head)
    k = k.view(-1, N_HEADS, D_HEAD).transpose(0, 1)  # (n_heads, seq, d_head)

    # Compute attention scores
    attn_scores = t.matmul(q, k.transpose(-2, -1)) / (D_HEAD ** 0.5)

    # Apply causal mask
    seq_len = attn_scores.shape[-1]
    causal_mask = t.triu(t.ones(seq_len, seq_len, device=attn_scores.device), diagonal=1).bool()
    attn_scores.masked_fill_(causal_mask, float('-inf'))

    # Apply softmax
    attn_patterns_value = t.softmax(attn_scores, dim=-1)
...
def calculate_h(
    model: StandardizedTransformer, dataset: ICLDataset, layer: int = -1
    ...
    with model.trace(dataset.prompts):
        h = model.layers_output[layer][:, -1].mean(dim=0).save()
        logits = model.logits[:, -1]
        next_tok_id = logits.argmax(dim=-1).save()

    completions = [model.tokenizer.decode(tok_id) for tok_id in next_tok_id.tolist()]
...
def intervene_with_h(
    ...
    with model.trace() as tracer:
        ...
        with tracer.invoke(zero_shot_dataset.prompts):
            token_completions_zero_shot = model.logits[:, -1].argmax(dim=-1).save()
        ...
        with tracer.invoke(zero_shot_dataset.prompts):
            ...
            model.layers_output[layer][:, -1] += h
            # Also save completions
            token_completions_intervention = model.logits[:, -1].argmax(dim=-1).save()

    # Decode to get the string tokens
    completions_zero_shot = [model.tokenizer.decode(tok_id) for tok_id in token_completions_zero_shot.tolist()]
    completions_intervention = [model.tokenizer.decode(tok_id) for tok_id in token_completions_intervention.tolist()]
...
def display_model_completions_on_h_intervention(
    ...
    for i in range(min(len(completions), num_to_display)):
        ...
        correct_completion = dataset.completions[i]
        correct_completion_first_token = tokenizer.tokenize(correct_completion)[0].replace(
            "\u0120", " "
        )
        seq = dataset.seqs[i]
...
def calculate_h_and_intervene(
    ...
    with model.trace() as tracer:
        with tracer.invoke(dataset.prompts):
            h = model.layers_output[layer][:, -1].mean(dim=0)

        with tracer.invoke(zero_shot_dataset.prompts):
            clean_tokens = model.logits[:, -1].argmax(dim=-1).save()

        with tracer.invoke(zero_shot_dataset.prompts):
            model.layers_output[layer][:, -1] += h
            intervene_tokens = model.logits[:, -1].argmax(dim=-1).save()

    completions_zero_shot = [model.tokenizer.decode(tok_id) for tok_id in clean_tokens.tolist()]
    completions_intervention = [model.tokenizer.decode(tok_id) for tok_id in intervene_tokens.tolist()]
    return completions_zero_shot, completions_intervention
...
def calculate_h_and_intervene_logprobs(
    ...
    with model.trace() as tracer:
        ...
        with tracer.invoke(zero_shot_dataset.prompts):
            clean_logprobs = model.logits.log_softmax(dim=-1)[
                range(len(zero_shot_dataset)), -1, correct_completion_ids
            ].save()

        with tracer.invoke(zero_shot_dataset.prompts):
            model.layers_output[layer][:, -1] += h
            intervene_logprobs = model.logits.log_softmax(dim=-1)[
                range(len(zero_shot_dataset)), -1, correct_completion_ids
            ].save()
...
def calculate_fn_vectors_and_intervene(
    ...
    correct_completion_ids = [toks[0] for toks in model.tokenizer(dataset.completions)["input_ids"]]
    ...
    with model.trace() as tracer:
        ...
        with tracer.invoke(dataset.prompts):
            for layer in layers:
                # Get hidden states (input to c_proj), reshape to get head dimension, store the mean tensor
                z = model.layers[layer].self_attn.c_proj.input[:, -1]
                z_reshaped = z.reshape(N, N_HEADS, D_HEAD).mean(dim=0)
                for head in heads:
                    z_dict[(layer, head)] = z_reshaped[head]

        # Run a forward pass on corrupted prompts, where we don't intervene or store activations
        ...
        for layer in layers:
            for head in heads:
                with tracer.invoke(corrupted_dataset.prompts):
                    # Get hidden states, reshape to get head dimension, then set it to the clean vector
                    z = model.layers[layer].self_attn.c_proj.input[:, -1]
                    z.reshape(N, N_HEADS, D_HEAD)[:, head] = z_dict[(layer, head)]
                    # Get logprobs at the end
                    logits = model.logits[:, -1]
...
def calculate_fn_vector(
    ...
    Inputs:
        model: StandardizedTransformer
            the transformer you're doing this computation with
        dataset: ICLDataset
            the dataset of clean prompts from which we'll extract the function vector
        head_list: list[tuple[int, int]]
    ...
    with model.trace(dataset.prompts):
        for layer, head_list_for_layer in head_dict.items():
            # Get the output projection layer (c_proj for GPT-2)
            c_proj = model.layers[layer].self_attn.c_proj

            # Get the mean output projection input (note, setting values of this tensor will not
            # have downstream effects on other tensors)
            hidden_states = c_proj.input[:, -1].mean(dim=0)
            ...
            # (we need a single batch dimension so we can use `c_proj`)
            # Note: GPT-2's c_proj has a bias, so we subtract it to get just the head outputs
            c_proj_output = c_proj(hidden_states.unsqueeze(0)).squeeze()
            c_proj_bias = c_proj.bias
            fn_vector_list.append((c_proj_output - c_proj_bias).save())
...
def intervene_with_fn_vector(
    ...
    with model.generate(max_new_tokens=n_tokens, repetition_penalty=1.2) as generator:
        with generator.invoke(prompt):
            tokens = model.generator.output.save()

        with generator.invoke(prompt):
            model.layers_output[layer][:, -1] += fn_vector
            tokens_intervention = model.generator.output.save()

    completion, completion_intervention = model.tokenizer.batch_decode(
...
if MAIN:
    ...
    # Note: these heads are found by running calculate_fn_vectors_and_intervene on GPT-2-XL
    # The specific heads will differ from GPT-J since this is a different model
    dataset = ICLDataset(_ANTONYM_PAIRS, size=20, n_prepended=5)

    # Use heads found from the CIE analysis on GPT-2-XL
    # These are the top heads by indirect effect (you should find these from your own analysis)
    head_list = [
        (8, 0),
        (8, 1),
        (9, 14),
...
# ==================================================
# SECTION 4: Steering Vectors in GPT2-XL
# ==================================================

# Note: Since we're already using GPT-2-XL throughout this notebook, we don't need
# to load a separate model. We just continue using the same model.
...
def calculate_and_apply_steering_vector(
    ...
    with model.generate(max_new_tokens=n_tokens, **SAMPLING_KWARGS) as generator:
        ...
        with generator.invoke(act_add_prompts):
            ...
            act_add_vectors = [
                model.layers_output[layer][i, -seq_len:]
        ...
        with generator.invoke(steered_prompts):
            ...
            for i, (layer, coeff, seq_len) in enumerate(
                ...
                model.layers_output[layer][:, :seq_len] += coeff * act_add_vectors[i]
