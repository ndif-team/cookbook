# Copied from ARENA 3.0 (https://github.com/callummcdougall/ARENA_3.0) with attribution.
# Original author: Callum McDougall
...
def _get_model_config(model, key):
    """Extract model config values from either TL-style (model.cfg.X) or plain attributes (model.X)."""
    if hasattr(model, "cfg"):
        return getattr(model.cfg, key)
    # For our plain PyTorch models or NNsight-wrapped models
    m = model._model if hasattr(model, "_model") else model
    return getattr(m, key)


def plot_comp_scores(
    model, comp_scores, title: str = "", baseline: Tensor | None = None, filename: str | None = None
):
    n_heads = _get_model_config(model, "n_heads")
    fig = px.imshow(
        to_numpy(comp_scores),
        y=[f"L0H{h}" for h in range(n_heads)],
        x=[f"L1H{h}" for h in range(n_heads)],
...
def plot_logit_attribution(
    model, logit_attr: Tensor, tokens: Tensor, title: str = "", filename: str | None = None
):
    tokens = tokens.squeeze()
    y_labels = convert_tokens_to_string(model, tokens[:-1])
    n_layers = _get_model_config(model, "n_layers")
    n_heads = _get_model_config(model, "n_heads")
    x_labels = ["Direct"] + [
...
def plot_neurons(
    ...
    neuron_numbers = einops.repeat(
        t.arange(_get_model_config(model, "d_model")), "n -> (s n)", s=data.starts_open.sum()
    )
    failure_types = einops.repeat(failure_types, "s -> (s n)", n=_get_model_config(model, "d_model"))
    data_open_proportion = einops.repeat(
        data.open_proportion[data.starts_open], "s -> (s n)", n=_get_model_config(model, "d_model")
    )
    df = pd.DataFrame(
        {
