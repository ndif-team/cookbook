# Adapted from ARENA 3.0 (https://github.com/callummcdougall/ARENA_3.0)
# Original author: Callum McDougall
# Adapted for NNsight + nnterp by the NDIF team.
#
# This file is pure data generation for the IOI task.
# It uses a HuggingFace tokenizer (no TransformerLens dependency).
...
def get_word_idxs(prompts, word_list, tokenizer):
    """Get the index of the words in word_list in the prompts."""
    idxs = []
    tokenized_words = [tokenizer.decode(tokenizer(word)["input_ids"][0]) for word in word_list]
    for prompt in prompts:
        toks = [
            tokenizer.decode(tok)
            for tok in tokenizer(prompt["text"], return_tensors="pt", padding=True)["input_ids"][0]
        ]
        idx = None
